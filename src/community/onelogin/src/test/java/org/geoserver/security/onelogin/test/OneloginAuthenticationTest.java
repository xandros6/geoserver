    ...
    private void confgiureFilter(PreAuthenticatedUserNameRoleSource serviceType) {
        try {
            ...
            if (config == null) {
                ...
                config.setMetadataURL("https://localhost:" + idpSamlService.httpsPort()
                        + METADATA_URL);
            }
            config.setUserGroupServiceName(serviceType == PreAuthenticatedUserNameRoleSource.RoleService ? "rs1"
                    : "ug1");
