...
/**
 * If the server has succesfully received all bytes from the operation,
 * it responds with a final status code; otherwise it responds with a 308 (Resume Incomplete),
 * indicating which bytes of the operation it has successfully received.
 */
public static final Status RESUME_INCOMPLETE = new Status(308);

public ResumableUploadCatalogResource(Context context, Request request, Response response, Catalog catalog, ResumableUploadResourceManager resumableUploadResourceManager) {
...
public void handlePut(){
    ...
    if(!contentRange.isEmpty()){
        try{
            ...
            if( startPosition > endPosition || ((endPosition - startPosition) != totalByteToUpload) ){
                getResponse().setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
                return;
            }
            /*
             * Validate resume request
             * If resume is requested existing file must contains the number of bytes matching startPosition
             */
